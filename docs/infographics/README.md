...
- 读者问题：readabilities-rs 的代码从哪里进入、由哪些分区构成、怎样运行并如何验证？
- 语言：简体中文；代码、路径与命令保留原文
- 媒介：2000 × 2962 静态矢量长图（高度按内容自适应）
...
- 源码规模：20 个源码文件、5238 个非空文本行
- 测试边界：15 个路径命中测试规则的已跟踪文件
...
google-chrome --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=1 --window-size=3000,4443 --virtual-time-budget=10000 --screenshot=architecture.png _render-png.html
